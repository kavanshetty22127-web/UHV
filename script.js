...
function updateAdminUsersList() {
    ...
    // Update users list with scores - ordered by who finished first
    usersList.innerHTML = '';
    
    // Create array of users with their scores and timestamps
    const usersWithScores = users.map(user => ({
        name: user,
        scoreData: userScores[user]
    }));
    
    // Sort by timestamp (earliest first) - users who finished first appear at top
    usersWithScores.sort((a, b) => {
        if (!a.scoreData) return 1; // No score goes to bottom
        if (!b.scoreData) return -1;
        return a.scoreData.timestamp - b.scoreData.timestamp;
    });
    
    // Display sorted users
    usersWithScores.forEach((userData, index) => {
        const listItem = document.createElement('li');
        const userScore = userData.scoreData;
        
        if (userScore) {
            listItem.innerHTML = `
                <strong>#${index + 1} - ${userData.name}</strong>
